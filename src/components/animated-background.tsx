...
const AnimatedBackground = () => {
  ...
  // Sound refs
  const pressSoundRef = useRef<HTMLAudioElement | null>(null);
  const releaseSoundRef = useRef<HTMLAudioElement | null>(null);

  useEffect(() => {
    pressSoundRef.current = new Audio("/assets/keycap-sounds/press.mp3");
    pressSoundRef.current.volume = 0.4;
    releaseSoundRef.current = new Audio("/assets/keycap-sounds/release.mp3");
    releaseSoundRef.current.volume = 0.4;
  }, []);
  ...
  const handleSplineInteractions = () => {
    ...
    splineApp.addEventListener("keyUp", () => {
      ...
      if (releaseSoundRef.current) {
        releaseSoundRef.current.currentTime = 0;
        releaseSoundRef.current.play().catch(() => {});
      }
      splineApp.setVariable("heading", "");
      splineApp.setVariable("desc", "");
    });
    splineApp.addEventListener("keyDown", (e) => {
      ...
      if (pressSoundRef.current) {
        ...
        pressSoundRef.current.play().catch(() => {});
      }
      const skill = SKILLS[e.target.name as SkillNames];
      if (skill) {
        setSelectedSkill(skill);
  ...
  const setupScrollAnimations = () => {
    ...
    createSectionTimeline("#skills", "skills", "experience", "top 70%");
